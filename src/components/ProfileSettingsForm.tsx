      ...
      <form onSubmit={submit} className="card" style={{ gap: "var(--space-3)" }}>
        ...
        <div className="field">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <input id="handle" className="input" value={value} onChange={(event) => setValue(event.target.value)} required minLength={3} maxLength={20} autoCapitalize="none" />
          </div>
        </div>
        <button type="submit" className="btn btn-primary" disabled={status === "saving"}>
          {status === "saving" ? "保存中…" : "変更を保存"}
        </button>
        {status === "error" && <p role="alert" style={{ color: "var(--color-accent-800)", fontSize: 13, margin: 0 }}>{message}</p>}
        {status === "saved" && (
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", flexWrap: "wrap" }}>
            <p role="status" className="text-muted" style={{ fontSize: 13, margin: 0 }}>{message}</p>
            <button type="button" className="btn btn-secondary" onClick={() => void refreshSessionAndReturnToProfile()}>もう一度反映</button>
          </div>
        )}
      </form>
